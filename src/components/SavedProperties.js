...
const SavedProperties = ({ userID }) => {
  ...
  useEffect(() => {
    axios.get(`https://sheltered-eyrie-56757.herokuapp.com/api/v1/Favourite?query={"fbUserId":"${userID}"}&populate=propertyListing`)
      .then((response) => {
  ...
  const handleRemoveProperty = (favouriteId) => {
    axios
      .delete(`https://sheltered-eyrie-56757.herokuapp.com/api/v1/Favourite/${favouriteId}`)
      .then((response) => {
